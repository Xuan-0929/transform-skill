...
def orchestrate_cmd(
    ...
    speaker: str | None = typer.Option(
        None,
        "--speaker",
        help="Optional speaker override. If omitted, agent can decide from corpus.",
    ),
    new_corpus_weight: float = typer.Option(
        0.25,
        "--new-corpus-weight",
        min=0.0,
        max=1.0,
        help="Default weight fallback for orchestration planning (0.0-1.0).",
    ),
    suite: Path | None = typer.Option(None, "--suite", help="Eval suite JSON path"),
    target: str = typer.Option("both", "--target", help="agentskills|codex|both|none"),
) -> None:
    """Agent-led, script-executed workflow: plan -> execute -> export."""
    if target not in {"agentskills", "codex", "both", "none"}:
        raise typer.BadParameter("--target must be agentskills|codex|both|none")
